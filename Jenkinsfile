pipeline {
    ...
    environment {
        ...
        Url_ZAP_Attack = 'http://34.204.15.181:8080/api/v1/employee/health'
    }
    ...
    stages{
        ...
        stage('Publish Report'){
            steps{
                publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, icon: '', keepAll: false, reportDir: '${WORKSPACE}', 
                reportFiles: 'ZAP-report.html', reportName: 'HTML Report', reportTitles: '', useWrapperFileDirectly: true])
            }
        }
    }
    post {
